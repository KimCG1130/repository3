...
public class UserDAO {
	...
	public void insertUser(UserVO userVO) throws Exception {
		...
		PreparedStatement stmt = con.prepareStatement(sql);//23번라인 sql 실행
		stmt.setString(1, userVO.getUserId());//ID
		stmt.setString(2, userVO.getUserName());//이름
		stmt.setString(3, userVO.getPassword());//비밀번호
		stmt.setString(4, userVO.getSsn());//주민등록번호
		stmt.setString(5, userVO.getPhone());//전화번호
		stmt.setString(6, userVO.getAddr());//주소
		stmt.setString(7, userVO.getEmail());//이메일주소
		stmt.executeUpdate();//아마도 반영된 레코드 수 값 반환해줄듯...?
